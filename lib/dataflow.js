...
exports.create = function (type, props) {
	...
	switch (typeof type) {
		case "string":
			...
			brick = new Constructor();
			break;

		case "object":
			brick = createComposite(type);
			break;

		default:
			throw new Error("Cannot create a brick from " + typeof type);
	}

	if (props) {
		Object.keys(props).forEach(function (key) {
			brick.props[key] = props[key];
		});
	}
